        ...
        <table border="1" width="100%" cellspacing="2" cellpadding="3">
            ...
            <xsl:for-each select="descendant::block[not(@sourceFile=preceding::block/@sourceFile)]">
                <xsl:sort data-type="number" order="descending"
                          select="sum(//block[@sourceFile=current()/@sourceFile]/parent::set/@lineCount)"/>
                <tr>
    ...
    <xsl:template match="check" mode="summary">
        <table border="1" width="100%" cellspacing="2" cellpadding="3">
            <tr class="a">
                ...
                <td>
                    <xsl:value-of select="//check/@threshold"/>
                </td>
            </tr>
            <tr class="b">
            ...
            <tr class="a">
                ...
                <td>
                    <xsl:value-of
                        select="format-number(sum(//set/@lineCount) div sum(//check/summary/@totalSignificantLineCount), '#.00%')"/>
                </td>
            </tr>
        </table>
    </xsl:template>
